...
var TestingManager = (
	function()
	{
		...
		/**
			...
			Is not a deep comparison, so inner property [] === [] returns false.
			will use the .equals() function supplied by the object, if it exists.
			@param the output 
		...
		{
			TestingManager.log(`error: ${this.name}`);
			TestingManager.log(`	${errorMessage}`);
		}
		...
		return {
			...
			/**
				...
				Since we are going to be using different engines, this increases portability.
				@author aivct
				@date 2022-11-13
			 */
